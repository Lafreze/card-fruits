...
export class FruitGame implements GameControls {
  ...
  private constructor(options: number | GameOptions, callbacks: GameCallbacks) {
    ...
    this.basketLeft += Math.max(0, tools.basket || 0);
    this.syrupLeft += Math.max(0, tools.syrup || 0);
    ...
    if (this.hasRelic("orchard_basket")) this.basketLeft += 1;
    if (this.hasRelic("sugar_kettle")) this.syrupLeft += 1;
  ...
  private async initialize(canvas: HTMLCanvasElement) {
    await Promise.all(
      FRUITS.flatMap((fruit) => (fruit.icon ? [Assets.load(fruit.icon)] : [])),
    );
    await this.app.init({
      canvas,
      width: WORLD.width,
    ...
    this.emitSnapshot();
    if (this.greenhouseSeedCount > 0) {
      const seedTier = Math.min(3, Math.floor(this.levelIndex / 7));
      for (let index = 0; index < this.greenhouseSeedCount; index += 1)
        this.setTimer(
          () =>
            this.spawnFruit(
              seedTier,
              WORLD.width / 2 +
                (index - (this.greenhouseSeedCount - 1) / 2) * 42,
              WORLD.box.y + 20,
            ),
          330 + index * 180,
        );
      this.setTimer(
        () =>
          this.callbacks.onToast(
            `🪴 育苗床 · ${FRUITS[seedTier].name}果种 ×${this.greenhouseSeedCount}`,
            "cyan",
          ),
        360 + this.greenhouseSeedCount * 180,
      );
    }
  ...
  private makeFruitIcon(tier: number, size: number, withGlow = false) {
    const fruit = FRUITS[tier];
    if (fruit.icon) {
      const icon = Sprite.from(fruit.icon);
      icon.anchor.set(0.5);
      icon.width = size;
      icon.height = size;
      return icon;
  ...
  private drawScene() {
    ...
    for (let index = 0; index < 22; index += 1) {
      const star = new Graphics()
        .circle(0, 0, 0.65 + Math.random() * 1.05)
        .fill({
  ...
  private launchFruits(limit = Number.POSITIVE_INFINITY, strength = 1) {
    ...
    if (candidates.length) {
      ...
      this.burst(
        WORLD.width / 2,
        WORLD.box.y + WORLD.box.height - 34,
        0x9de9ff,
        28,
      );
      sounds.launch();
  ...
  private createCards(definition = LEVELS[this.levelIndex]) {
    ...
    const cardCount = definition.cards.reduce(
  ...
  private makeCard(tier: number, special: CardSpecial, layer = 0) {
    ...
    const glowInner = new Graphics()
      .roundRect(-CARD_W / 2 - 2, -CARD_H / 2 - 2, CARD_W + 4, CARD_H + 4, 14.5)
    ...
    const paperWash = new Graphics()
      .roundRect(-CARD_W / 2 + 2, -CARD_H / 2 + 2, CARD_W - 4, CARD_H - 4, 10.5)
    ...
    const fruitIcon = this.makeFruitIcon(tier, fruit.icon ? 37 : 31);
    fruitIcon.position.set(0, -2.5);
    fruitIcon.label = "card-symbol";
    ...
    if (special === "frozen") {
      ...
      this.addCardBadge(view, "🍬", 0xb88937, "special-icon");
    } else if (special === "harvest") {
      this.addCardBadge(view, "🌾", 0x8b8145, "special-icon");
    } else if (special === "prism") {
      this.addCardBadge(view, "🔮", 0x7868a4, "special-icon");
    } else if (special === "gust") {
      this.addCardBadge(view, "🌪️", 0x4f7e8c, "special-icon");
    }
    return view;
  }
  ...
  private pickCard(card: CardNode) {
    ...
    if (
      ...
      this.hasRelic("fruit_pinata") &&
      this.pickedCardCount % 10 === 0
    ) {
      const pinataScore = 480 + this.wave * 120;
      this.gainFever(12);
      this.addScore(pinataScore, card.x, card.y);
      [0xff6fae, 0x7ce7ff, 0xffdb69].forEach((color, index) =>
        this.setTimer(() => this.burst(card.x, card.y, color, 13), index * 45),
      );
      this.callbacks.onToast(
        `🪅 水果彩罐 · +${pinataScore} 分 · 甜度 +12`,
        "pink",
      );
    }
    ...
    if (launchesFruit) {
      ...
      if (launched)
        this.callbacks.onToast(`🌪️ 季风牌 · 弹开 ${launched} 颗水果`, "cyan");
    }
    ...
    for (let index = 0; index < this.trayLimit; index += 1) {
      ...
      if (tier !== undefined) {
        const fruitIcon = this.makeFruitIcon(
          tier,
          FRUITS[tier].icon ? 31 : this.trayLimit > 7 ? 23 : 26,
        );
        fruitIcon.position.set(x, WORLD.tray.y + WORLD.tray.height / 2 - 1);
        this.trayLayer.addChild(fruitIcon);
      }
    }
  }
  ...
  private startConversion(tier: number, matchedSlots: number[]) {
    ...
    const cards = starts.map(({ x, y }) => {
      ...
      const face = new Graphics()
        ...
        .fill({ color: 0xffffff });
      const icon = this.makeFruitIcon(tier, FRUITS[tier].icon ? 20 : 16);
    ...
    const fruit = this.makeFruitIcon(tier, FRUITS[tier].icon ? 31 : 26, true);
  ...
  private queueFruitRain(tier: number, power = this.dropPowerForTier(tier)) {
    ...
    if (
      this.mode === "expedition" &&
      this.hasRelic("triple_crown") &&
      this.cardMatchCount > 0 &&
      this.cardMatchCount % 5 === 0
    ) {
      mainTier = Math.min(FRUITS.length - 1, mainTier + 1);
      crownBoost = true;
    }
    ...
    if (crownBoost)
      this.callbacks.onToast(
        `👑 三消王冠 · ${FRUITS[mainTier].name}`,
        "gold",
      );
    else if (mainTier > tier)
      this.callbacks.onToast(
  ...
  private makeRainPreview(tier: number, total: number) {
    const view = new Container();
    const glow = new Graphics().circle(0, 0, 24).fill({
    ...
    const fruitIcon = this.makeFruitIcon(
      tier,
      FRUITS[tier].icon ? 29 : 23,
      true,
    );
    fruitIcon.position.set(0, -1);
    view.addChild(glow, fruitIcon);
  ...
  private updateFruitRain() {
    ...
    this.fruitRain.forEach((drop) => {
      ...
      if (timeLeft > 0) {
        ...
        drop.view.y = WORLD.box.y + 28 - Math.sin(ready * Math.PI) * 7;
        remaining.push(drop);
        return;
      }
      ...
      const impactPower = Math.min(
        3,
        Math.max(drop.power, drop.total) + Number(this.hasRelic("shock_core")),
      );
  ...
  private linkFusionPair(first: FruitNode, second: FruitNode, power = 1) {
    if (
      first === second ||
  ...
  private triggerDropImpact(node: FruitNode) {
    ...
    const radius = (72 + power * 34) * (this.hasRelic("storm_stir") ? 1.25 : 1);
  ...
  private mergeFruits(first: FruitNode, second: FruitNode) {
    ...
    const meteor =
      this.mode === "expedition" &&
      this.hasRelic("meteor_garden") &&
      this.physicalMergeCount % 4 === 0;
    if (confetti) {
      this.gainFever(12);
      [0xff6fae, 0x7ce7ff, 0xffdb69].forEach((color, index) =>
    ...
    if (meteor) {
      this.gainFever(10);
      this.setTimer(() => {
        const launched = this.launchFruits(5, 1.12);
        this.callbacks.onToast(
          `☄️ 流星果园 · 弹射 ${launched} 颗 · 甜度 +10`,
          "cyan",
        );
      }, 120);
    }
  ...
  private snapshot(): GameSnapshot {
    return {
      ...
      basketLeft: this.basketLeft,
      syrupLeft: this.syrupLeft,
      wave: this.wave,
      mode: this.mode,
      relics: [...this.relics],
  ...
  shuffle = () => {
    ...
    const preferredTier = [
      ...new Set([...trayCounts.keys(), ...activeCounts.keys()]),
    ]
      .map((tier) => ({
        tier,
  ...
  hammer = () => {
    ...
    this.callbacks.onToast(
      `清顶锤 · 自动收取${FRUITS[target.tier].name}`,
      "gold",
    );
  ...
  wild = () => {
    ...
    this.wildLeft -= 1;
    this.callbacks.onToast(`万能果 · 补齐${FRUITS[tier].name}三消`, "gold");
  ...
  bubble = () => {
    ...
    ordered.forEach(({ index }) => {
      const tier = this.tray[index];
      this.bubbleGroups.set(tier, ((this.bubbleGroups.get(tier) || 0) + 1) % 3);
      this.tray.splice(index, 1);
    });
    this.bubbleLeft -= 1;
    this.lastPick = null;
  ...
  ripen = () => {
    ...
    const target = [...this.fruits.values()]
      ...
      .sort(
        (a, b) => a.tier - b.tier || b.body.position.y - a.body.position.y,
      )[0];
    if (!target) {
  ...
  split = () => {
    ...
    const target = [...this.fruits.values()]
      ...
      .sort(
        (a, b) => b.tier - a.tier || a.body.position.y - b.body.position.y,
      )[0];
    if (!target) {
  ...
  // 穿层采摘篮：优先选择当前露出最多的一组同果卡，再从高层补齐三张。
  // 它直接转果、不占卡槽，适合卡槽高压或牌阵入口被打散时救场。
  basket = () => {
    if (this.paused || this.status !== "playing" || this.basketLeft <= 0)
      return;
    if (this.cardToolBlocked()) return;
    const groups = new Map<number, CardNode[]>();
    this.cards.forEach((card) => {
      if (!card.active) return;
      const group = groups.get(card.tier) || [];
      group.push(card);
      groups.set(card.tier, group);
    });
    const candidate = [...groups.entries()]
      ...
      .map(([tier, cards]) => ({
        tier,
        cards,
        exposed: cards.filter(
          (card) => !card.locked && !this.isCovered(card),
        ).length,
      }))
      .sort(
        (first, second) =>
          second.exposed - first.exposed ||
          second.tier - first.tier ||
          second.cards.length - first.cards.length,
      )[0];
    if (!candidate) {
      this.callbacks.onToast("牌堆里没有完整的三张同果卡", "cyan");
      return;
    }
    const sources = [...candidate.cards]
      .sort(
        (first, second) =>
          Number(this.isCovered(first) || first.locked) -
            Number(this.isCovered(second) || second.locked) ||
    ...
    sources.forEach((card) => {
      ...
      card.view.visible = false;
      this.unlockNearby(card);
      this.burst(card.x, card.y, FRUITS[candidate.tier].glow, 10);
    });
    this.lastPick = null;
    this.startConversion(candidate.tier, []);
    this.updateCardAccess();
    this.drawTray();
    this.maybeFinishStory();
    this.callbacks.onToast(
      `🧺 穿层采摘 · ${FRUITS[candidate.tier].name}三张转果`,
      "gold",
    );
    haptic([14, 22, 14, 28, 22]);
    this.emitSnapshot();
  };

  syrup = () => {
    if (this.paused || this.status !== "playing" || this.syrupLeft <= 0)
      return;
    this.syrupLeft -= 1;
    if (this.feverActive) {
      this.feverUntil += 2.5;
      this.callbacks.onToast("🍯 甜度糖浆 · 狂热延长 2.5 秒", "gold");
    } else {
      this.gainFever(45);
      if (!this.feverActive)
        this.callbacks.onToast("🍯 甜度糖浆 · 甜度 +45", "pink");
    }
    this.burst(WORLD.width / 2, WORLD.tray.y + 42, 0xffd166, 28);
    this.ring(WORLD.width / 2, WORLD.tray.y + 42, 0xffe59a, 0.9);
    haptic([12, 22, 18]);
    this.emitSnapshot();
  };
